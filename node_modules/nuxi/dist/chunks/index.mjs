...
import { g as gray, c as cyan, u as underline, b as bold } from '../shared/nuxi-edge.d4f35867.mjs';
...
async function listen(handle, opts = {}) {
  ...
  let server;
  let addr;
  const getURL = (host, baseURL) => `${addr.proto}://${host || addr.addr}:${addr.port}${baseURL || opts.baseURL}`;
  if (opts.https) {
    const { key, cert } = await resolveCert({ ...opts.https });
...
async function resolveCert(opts) {
